...
with st.sidebar:
    ...
    if session is None:
        session = (
            utils.getLocalSession() if utils.isLocal() else utils.getRemoteSession()
        )
    ...
    if hasTable:
        ...
        st.caption("test.public.emp_dept")
    else:
        tableName = "test.public.employees"  # default value, if empty input
        st.caption("test.public.emp_dept")
...
# show the path
with tabPath:
    # indexes of selected columns (+1 because list index starts at 0)
